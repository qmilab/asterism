...
- **A question Asterism needs answered is now always put where you can see it.** Every prompt — the destructive-action confirmation, `reflect --review`, `trust --review` — is written to **standard error**, and Asterism asks only when standard error is a terminal as well as standard input.
  ...
  What a question is *about* now goes with it. The three review commands printed each item — a capability and the record that earned it, a proposed memory and its firewall warnings — on standard output while asking about it on standard error, so `asterism trust bot --review > out.txt` at a terminal asked you to grant a permanent auto-approve for a destructive capability whose name had gone into the file, and `asterism reflect writer --review > out.txt` asked which objective to retire without showing which. The whole review is on standard error now; only the closing count is a result.
