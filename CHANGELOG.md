...
- **`asterism config show` names the layer a model actually comes from.** The `[agent override]` / `[environment]` / `[install default]` label was derived beside the resolver rather than from it, and each of its clauses tested whether its layer had the field *at all* — so a layer holding an empty value was credited with a model that came from somewhere else. It is now read off the same layers resolution uses.
