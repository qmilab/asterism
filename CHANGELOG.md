...
- **An option given only whitespace is the same error as an option given nothing.** `asterism config set "  "` used to report a model set, and the very next `asterism config show` reported none — the value was accepted by the command and discarded by everything that read it. The writer now refuses exactly what the reader would throw away. Padding *around* a value you typed is still yours: `--provider " openai "` is stored as typed.
