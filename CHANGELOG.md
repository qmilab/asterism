...
- **The places that merely mention the gate now say what they actually mean.** The container, service, `serve`, `channel`, `service install` and cognition-trace passages were each making a point about *their own surface* — that running in a container, or as a background service, or with a trace recording, does not loosen anything — and they made it by restating the gate, which put half the rule in a dozen more places to keep in step. They now say the thing they were always for: the gate reaches the verdict it would have reached at the keyboard, whatever that verdict is.
  ...
  Their unattended-run sentences went with them. Each said an HTTP run *parks at `awaiting_confirmation`* when nobody is watching — but if the capability is one you allowed, the run does not park at all, it just goes. They now say that a run **the gate stops** parks, which is the case they were describing.
...
- **A question Asterism needs answered is now always put where you can see it.** Every prompt — the destructive-action confirmation, `reflect --review`, `trust --review` — is written to **standard error**, and Asterism asks only when standard error is a terminal as well as standard input.
  ...
  The two review commands no longer decide on your behalf when there is nobody there to decide. `asterism trust <agent> --review` used to walk you through the evidence for each earned capability and end with `0 granted, N left gated` — a summary of decisions nobody had made; it now names how many are waiting and stops. `asterism reflect <agent> --review` used to ask the model for proposals and reject every one of them, so an unattended run paid for an answer it then discarded; it now says so before the call. Both point at the terminal, or at `asterism reflect <agent> --propose`, which is the mode built to run unattended. Nothing they would have written is lost.

  What a question is *about* now goes with it. All three reviews — earned capabilities, proposed memories, and suggested objective changes — printed each item on standard output while asking about it on standard error, so `asterism trust bot --review > out.txt` at a terminal asked you to grant a permanent auto-approve for a destructive capability whose name had gone into the file, and `asterism reflect writer --review > out.txt` asked which objective to retire without showing which. The whole review is on standard error now; only the closing count is a result.
...
- **An environment variable that exists but is empty is read as unset, not as set to nothing.** `export ASTERISM_MODEL_ID=` is how a shell clears a variable, and Asterism used to read the cleared variable as an instruction. The two commands that answer "which model will run" then answered differently about the same install: `asterism config show` reported a configured model *and* an active override, while `asterism run` reported no model at all. An emptied variable now supplies nothing and the configured model shows through — the same reading the HTTP and chat-channel tokens always had, and the one `asterism secrets add` adopted in 0.9.0.
  ...
  `asterism config show` also stopped calling a *half*-configured local-embeddings endpoint configured. It reported one whenever either of `ASTERISM_RECALL_EMBED_URL` / `ASTERISM_RECALL_EMBED_MODEL` was set, where a run on an agent using that provider needs both and refuses without them — so the line said configured and the run said not configured. Both surfaces now read the same answer, and the half-configured case says which variable is still missing rather than falling silent over one you did set.

- **`asterism config show` names what an environment override actually beats.** It said `ASTERISM_MODEL_ID set — overrides the config file`, directly above per-agent lines showing an agent running on the model pinned for it *in that file*. An agent's own model wins over the environment; the line now says so.

- **`asterism config unset` says what it removed.** A stored entry that supplies nothing — `{"provider": ""}`, which an earlier version would write — is still removed, because that is the way out of one, but it is no longer reported as a cleared setting when `asterism config show` has just reported nothing set. Both the install default and a per-agent override.

- **`asterism config show` describes one model per line, not one per reader.** Two lines disagreed with the rest of the same output when a stored coordinate was empty. The `[agent override]` / `[environment]` / `[install default]` label was derived beside the resolver rather than from it, and each of its clauses tested whether its layer had the field *at all* — so a layer holding an empty value was credited with a model that came from somewhere else. And the install-default line printed the config file verbatim — `llama3.2 (provider: , base url: )` — directly above per-agent lines reporting what those same coordinates resolve to. Both are now read off the layers resolution itself uses.
...
- **An option given only whitespace is the same error as an option given nothing.** `asterism config set "  "` used to report a model set, and the very next `asterism config show` reported none — the value was accepted by the command and discarded by everything that read it. The writer now refuses exactly what the reader would throw away, on every option, including the ones with refusals of their own: `asterism api add … --credential "  "` used to bind an endpoint to a credential key of two spaces, and `asterism channel telegram bot --allow "  "` (or `--allow ","`) started the bot with **no allow-list** where one had been named. Padding *around* a value you typed is still yours: `--provider " openai "` is stored as typed.
...
- **An option that carries nothing is now named before a missing argument is.** `asterism new --soul` and `asterism api add --credential` reported the usage line, sending you to look for arguments you had not typed yet rather than at the option that was wrong. They now name the option, which is the rule a misspelled option has followed since 0.8.0. An option you did not type at all is a different mistake and still gets the usage line, which is the thing that names the whole shape of the command.

### Documentation

- **The npm page for `@qmilab/asterism` described the 0.1.0 product.** It is the page you read before deciding to install anything, and everything shipped since the first release was missing from it: the live [dashboard](https://qmilab.com/asterism/docs/dashboard/), [Telegram and Discord channels](https://qmilab.com/asterism/docs/channels/), running as a [background service](https://qmilab.com/asterism/docs/service/), the token-protected [HTTP endpoint](https://qmilab.com/asterism/docs/http/), the multi-arch [container image](https://qmilab.com/asterism/docs/container/), the governed-learning loop — [recall](https://qmilab.com/asterism/docs/concepts/#recall), [earned trust](https://qmilab.com/asterism/docs/concepts/#earned-autonomy), standing objectives and working notes — the five ways two agents can [work together](https://qmilab.com/asterism/docs/collaboration/), and [model access](https://qmilab.com/asterism/docs/models/) by provider name. Nothing on the page was false; it had simply stopped keeping up. It now covers what Asterism does, shows the destructive-action gate as it actually prints, and says plainly what "separate" does and does not mean today — logical scoping, not OS-level containment. That last passage the repo's front page and the docs have carried for several releases; the page most people read first never had it.

- **A link written as a whole URL into this project's own site is now checked like any other.** A markdown page cannot always write a relative link: the npm page is rendered by npm, where `./docs/x.md` resolves against the repository rather than the site, so its links have to name the site outright — and an absolute URL was exactly the shape every link check set aside as somebody else's. `bun run check:docs` now resolves them against what the site actually serves, pages and files alike, headings included, and reads `use_directory_urls` from `mkdocs.yml` rather than accepting whichever URL shape a link happens to use. That is what makes the rewritten npm page's every link — its screenshot included — a claim something checks rather than one nobody reads until it 404s.
