...
// The default interface (loopback) is defined once in `http.js` and shared by both
// surfaces; re-export it here so it stays part of this package's public API.
export { DEFAULT_HOSTNAME } from "./http.js";
...
// The install-wide operator console — the second HTTP surface, and the thin backend
// the `asterism dashboard` TUI is a client of. Re-exported here so the package has a
// single entry point. (console.ts imports only `RunningServer` as a type and the
// loopback default from http.ts, so there is no runtime import cycle.)
export {
  handleConsoleRequest,
  serveConsole,
  DEFAULT_CONSOLE_PORT,
} from "./console.js";
export type { ConsoleDeps, ServeConsoleOptions } from "./console.js";
