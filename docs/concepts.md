...
[destructive-action gate](#the-destructive-action-gate) — whatever an agent's
trust level, a deletion never happens on its own: `notify` and `autonomous` stop
and ask, and `propose` does not attempt it — not the workspace boundary. Stronger execution
